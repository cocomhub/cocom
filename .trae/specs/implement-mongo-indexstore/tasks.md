...
- [x] 任务 7：ComicInfo 场景影响面与兼容格式治理
  - [x] 子任务 7.1：梳理 `comicInfo` 现有 `archive` 字段格式与读路径，确认必须保留的旧字段
  - [x] 子任务 7.2：调整 `NewComicInfoArchiveIndexStore` 的编解码/过滤设计，使 manager 元数据与旧 `ArchiveInfo` 兼容共存
  - [x] 子任务 7.3：限制写入影响范围，仅允许修改 `archive` 子树且 `cid` 不存在时不得插入稀疏 `comicInfo` 文档
  - [x] 子任务 7.4：补充兼容性测试，覆盖“旧字段仍可读”“非 archive 字段不变”“缺失 cid 返回错误”
