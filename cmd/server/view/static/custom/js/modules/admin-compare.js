...
(function () {
  ...
  function renderCompareResult(data) {
    ...
    (data.comparison || []).forEach(function (row) {
      ...
      var cid1md5 = row.cid1_md5 ? row.cid1_md5.substring(0, 12) + '...' : '<span style="color:#ffc107;">无</span>';
      var cid2md5 = row.cid2_md5 ? row.cid2_md5.substring(0, 12) + '...' : '<span style="color:#ffc107;">无</span>';
      var cid2Color = ok ? 'style="color:#666;"' : 'style="color:#f44336;"';
      html += '<tr' + cls + '>' +
        '<td style="padding:4px 10px;">' + escapeHtml(row.name) + '</td>' +
        '<td style="padding:4px 10px;font-family:monospace;font-size:11px;color:#666;">' + cid1md5 + '</td>' +
        '<td style="padding:4px 10px;font-family:monospace;font-size:11px;' + cid2Color + '">' + cid2md5 + '</td>' +
