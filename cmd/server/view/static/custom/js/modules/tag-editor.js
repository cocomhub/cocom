...
(function() {
  ...
  function buildTagEditorModal(cid, currentTags) {
    ...
    addBtn.onclick = function() {
      ...
      if (!name) { window.showToast('请输入 tag 名称', { type: 'error' }); return; }
    ...
    var wrapper = window.showCustomModal('Edit Tags', modalContent,
        ...
        saveXhr.onload = function() {
          ...
          if (saveXhr.status >= 200 && saveXhr.status < 300) {
            ...
            try {
              ...
              window.showToast(r.head && r.head.msg || '保存失败', { type: 'error' });
            } catch (e) {
              window.showToast('保存失败: ' + saveXhr.status, { type: 'error' });
            }
          }
        };
