# 文档目录

## 概览
- [架构总览](overview/architecture.md)

## 运维与配置
- [配置管理](operations/configuration.md)
- [部署与运行](operations/deployment.md)

## 命令行工具（CLI）
- [server](cli/server.md)
- [verify](cli/verify.md)
- [image](cli/image.md)
- [install](cli/install.md)
- [version](cli/version.md)

## API 参考
- [Legacy `/api/*` 接口](api/legacy.md)
- [V2 `/v2/api/{onecomic|nhcomic}` 接口](api/v2.md)
- [调试与管理接口](api/debug.md)

## Web 界面
- [页面与路由](ui/web.md)

## 模块参考
- [Comic 模块](modules/comic.md)
- [Imaging 模块](modules/imaging.md)
- [Download 模块](modules/download.md)
- [Logging 模块](modules/logging.md)
- [Storage 模块](modules/storage.md)

## 指南与示例
- [故障排查](troubleshooting.md)
- [使用示例](examples.md)
