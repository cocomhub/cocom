...
func decode(r io.Reader, configOnly bool) (image.Image, image.Config, error) {
	...
	if formType != fccWEBP {
		return nil, image.Config{}, fmt.Errorf("%w: %v", errInvalidFormat, formType)
	}
	...
	for {
		...
		if err == io.EOF {
			err = fmt.Errorf("%w: riffReader 读取到文件末尾", errInvalidFormat)
		}
		if err != nil {
			return nil, image.Config{}, err
		}
		...
		case fccALPH:
			if !wantAlpha {
				return nil, image.Config{}, fmt.Errorf("%w: %v fccALPH 格式错误 wantAlpha=%v", errInvalidFormat, chunkID, wantAlpha)
			}
			wantAlpha = false
		...
		case fccVP8:
			if wantAlpha || int32(chunkLen) < 0 {
				return nil, image.Config{}, fmt.Errorf("%w: %v fccVP8 格式错误 wantAlpha=%v chunkLen=%d", errInvalidFormat, chunkID, wantAlpha, chunkLen)
			}
			d := vp8.NewDecoder()
		...
		case fccVP8X:
			if seenVP8X {
				return nil, image.Config{}, fmt.Errorf("%w: %v fccVP8X 格式错误 重复出现", errInvalidFormat, chunkID)
			}
			seenVP8X = true
			if chunkLen != 10 {
				return nil, image.Config{}, fmt.Errorf("%w: %v fccVP8X 格式错误 chunkLen=%d", errInvalidFormat, chunkID, chunkLen)
			}
...
func readAlpha(chunkData io.Reader, widthMinusOne, heightMinusOne uint32, compression byte) (
	...
	return nil, 0, fmt.Errorf("%w: readAlpha 格式错误 compression=%d", errInvalidFormat, compression)
}
